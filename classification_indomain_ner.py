...
def make_loader(train_data, test_data, ner_type, ner_data):
    with open(ner_data, mode='rb') as f:
        nerN = pickle.load(f)
    nerN = nerN[0]

    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case= True)
    num_added_toks = tokenizer.add_tokens(nerN)
    # 'NER TYPE: 0: NER, 1: del, 2:Wikid, 3:WikiD+Del, 4: WikiD+NER'
    if ner_type == 0:
        get_text = 'replace'
    elif ner_type == 1:
        get_text = 'remove_sentence'
    elif ner_type == 2:
        get_text = 'USER_s'
    elif ner_type == 3:
        get_text = 'rem_ner'
...
def main():
    ...
    parser.add_argument('--test', dest='Test Data', type=int)
    parser.add_argument('--ner_type', dest= 'NER TYPE: 0: NER, 1: del, 2:Wikid, 3:WikiD+Del, 4: WikiD+NER')
    parser.add_argument('--ner_data', dest = 'NER data')
    ...
    ner_type = parser.ner_type
    ner_data = parser.ner_data
    epochs = parser.n_epoch
    output_dir = parser.output

    train_loader, test_loader, tokenizer = make_loader(train_data, test_data, ner_type, ner_data)
